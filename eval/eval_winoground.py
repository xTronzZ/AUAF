...
Dataset: Winoground
Size:
    - Full: 400 pairs (800 images, 800 texts).
    - Very small, manually curated.
Purpose: Fine-grained Semantics / Hard Negatives / Compositionality
    - Contains image-text pairs with identical words but different word orders (e.g., "Person sits on grass" vs "Grass sits on person").
    - Role: Tests for "blind confidence". Current models often fail here (near-random accuracy). A successful uncertainty method should flag these samples with high uncertainty, preventing the model from making "confident but wrong" predictions on tricky compositional queries.
"""
...
from dataloaders import WinogroundDataLoader
from metrics import *
...
def eval_single(modelname, dataloader, device, method='score_top1var'):
    ...
    if os.path.isfile(f'results/embeddings_winoground_single_{modelname}.pt'):
        print (f'Loading pre-computed embeddings from results/embeddings_winoground_single_{modelname}.pt')
        all_txt_embs, all_img_embs, truth, all_scores = safe_torch_load(f'results/embeddings_winoground_single_{modelname}.pt', map_location=device)
    else:
        all_img_embs = []
        all_txt_embs = []
        all_scores = []
        ...
        torch.save([all_txt_embs, all_img_embs, truth, all_scores], f'results/embeddings_winoground_single_{modelname}.pt')
        print(f'Saved embeddings into results/embeddings_winoground_single_{modelname}.pt')
...
def eval_MCD(modelnames, dataloader, device, n_passes=50, method='score_top1var'):
    ...
    for modelname in modelnames:
        ...
        if os.path.isfile(f'results/embeddings_winoground_montecarlo_{modelname}.pt'):
            print (f'Loading pre-computed embeddings from results/embeddings_winoground_montecarlo_{modelname}.pt')
            all_txt_embs, all_img_embs, truth, all_scores = safe_torch_load(f'results/embeddings_winoground_montecarlo_{modelname}.pt', map_location=device)
            print('Done!')
        else:
            all_img_embs = []
            all_txt_embs = []
            ...
            torch.save([all_txt_embs, all_img_embs, truth, all_scores], f'results/embeddings_winoground_montecarlo_{modelname}.pt')
            print(f'Saved embeddings into results/embeddings_winoground_montecarlo_{modelname}.pt')
...
def eval_ensemble(modelnames, datasets, dataloader, device, method='score_top1var'):
    ...
    metrics_calibration = {}
    for modelname in modelnames:
        if os.path.isfile(f'results/embeddings_winoground_ensemble_{modelname}.pt'):
            print (f'Loading pre-computed embeddings from results/embeddings_winoground_ensemble_{modelname}.pt')
            all_txt_embs, all_img_embs, truth, all_scores = safe_torch_load(f'results/embeddings_winoground_ensemble_{modelname}.pt', map_location=device)
        else:
            all_img_embs = []
            all_txt_embs = []
            all_scores = []
            ...
            torch.save([all_txt_embs, all_img_embs, truth, all_scores], f'results/embeddings_winoground_ensemble_{modelname}.pt')
            print(f'Saved embeddings into results/embeddings_winoground_ensemble_{modelname}.pt')
...
if __name__ == '__main__':
    # Use WinogroundDataLoader
    dataloader = WinogroundDataLoader(auth_token=None, batch_size=32).get_dataloader()
    device = "cuda" if torch.cuda.is_available() else "cpu"

    out_dir = "results"
    os.makedirs(out_dir, exist_ok=True)

    if os.path.isfile('results/baselines_winoground.json'):
        print ('Loading pre-computed metrics from results/baselines_winoground.json')
        with open('results/baselines_winoground.json') as f:
            metrics = json.load(f)
    else:
        print ('No pre-computed metrics found (results/baselines_winoground.json). Computing...')
        metrics = eval_baselines(["ViT-B-32", "ViT-B-16", "ViT-L-14", "ViT-L-14-336"], dataloader, device)
        print ('Saved computed metrics into results/baselines_winoground.json')
        with open('results/baselines_winoground.json', 'w') as f:
            json.dump(metrics,f)
    ...
    for method in ['top1similarity','top1consistency','adversarial','adversarial_lin']:
        ...
        with open(f'results/metrics_rejection_winoground_single_{method}.json', 'w') as f:
            json.dump(metrics_rejection, f)
        with open(f'results/metrics_calibration_winoground_single_{method}.json', 'w') as f:
            json.dump(metrics_calibration, f)
        with open(f'results/oracle_metrics_rejection_winoground_single.json', 'w') as f:
        ...
        with open(f'results/metrics_rejection_winoground_montecarlo_{method}.json', 'w') as f:
            json.dump(metrics_rejection, f)
        with open(f'results/metrics_calibration_winoground_montecarlo_{method}.json', 'w') as f:
            json.dump(metrics_calibration, f)
        ...
        if method == 'score_top1avg': print_metrics(metrics_rejection)
        with open(f'results/metrics_rejection_winoground_ensemble_{method}.json', 'w') as f:
            json.dump(metrics_rejection, f)
        with open(f'results/metrics_calibration_winoground_ensemble_{method}.json', 'w') as f:
            json.dump(metrics_calibration, f)

    print_calibration_table_stdout("Winoground", TABLE)
